...
class TestSwitchResolution(WebrtcSdkTestCase):
  ...
  def testFirstFrame(self):
    ...
    self.logger().info("testFirstFrame start, options: {}", TestSwitchResolution.Options)
    ...
    self.assertTrue(
      self.waitAndCheck(TestSwitchResolution.Options["testFirstFrameMaxWaitTime"], checkFun)
    )
    ...
    for i in range(TestSwitchResolution.Options["testFirstFrameCount"]):
