...
class TestSwitchResolutionNetwork(WebrtcSdkTestCase):
  ...
  def testFirstFrame(self):
    ...
    self.logger().info("testFirstFrame start, options: {}", TestSwitchResolutionNetwork.Options)
    ...
    self.assertTrue(
      self.waitAndCheck(TestSwitchResolutionNetwork.Options["testFirstFrameMaxWaitTime"], checkFun)
    )
    ...
    for i in range(TestSwitchResolutionNetwork.Options["testFirstFrameCount"]):
